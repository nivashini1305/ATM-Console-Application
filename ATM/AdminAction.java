...
public class AdminAction {
    ...
    public static void depositMoney(Scanner s,Admin currentAdmin){
        ...
        if(depositAmount==denominationamount){//checks calculated amount and deposited amount
            ...
            currentAdmin.getTransactions().add(new Transactions( "deposited", ATM.getBalance()));//adds the object of Transaction
        }
        else {
            ...
            case 1:
                if (!currentAdmin.getTransactions().isEmpty()) {
                    ...
                    for (Transactions transactions : currentAdmin.getTransactions()) {//loops the admin transaction
                        System.out.println(currentAdmin.getUserId() + " has " + transactions.getTypeOfTransaction() + " the amount of " + transactions.getBalance());//prints the admin transaction
                    }
                    return;
                }
                ...
                for (Accounts user : ATM.getAccounts()) {
                    if (user instanceof User) {
                        if (user.getUserId().equals(userName)) {
                            for (Transactions transactions : user.getTransactions()) {//loops the particular user transaction
                                System.out.println("The transaction of " + user.getUserId() + " has " + transactions.getTypeOfTransaction() + " the amount of " + transactions.getBalance());//prints the user transaction
                            }
                        }
                        System.out.println("Enter user name correctly");
                        return;
                    }
                }
                ...
                for(Accounts admin:ATM.getAccounts()) {
                    if (admin instanceof Admin) {
                        for (Transactions transactions : currentAdmin.getTransactions()) {//loops for admin transaction
                            System.out.println(currentAdmin.getUserId() + " has " + transactions.getTypeOfTransaction() + " the amount of " + transactions.getBalance());
                        }
                    }
                }
                ...
                for(Accounts user:ATM.getAccounts()) {//loops all the user transaction and prints
                    if (user instanceof User) {
                        for (Transactions transactions : user.getTransactions()) {
                            System.out.println("The transaction of " + user.getUserId() + " has " + transactions.getTypeOfTransaction() + " the amount of " + transactions.getBalance());
                        }
                    }
                }
            }
